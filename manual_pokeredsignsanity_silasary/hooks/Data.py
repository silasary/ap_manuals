...
def after_load_region_file(region_table: dict) -> dict:
    from worlds.pokemon_rb.regions import warp_data
    for region in warp_data.keys():
        if region == "Menu":
            continue
        if region not in region_table:
            region_table[region] = {"requires": f"{{MainGameCanReachRegion({region})}}"}
    return region_table
